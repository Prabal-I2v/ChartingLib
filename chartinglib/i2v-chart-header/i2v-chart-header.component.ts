...
import { ICustomFilter, ITimeRange, ICustomFilterOutputEmittorModel, CustomFilterValueModel, IDateTimeFilterOutputEmittorModel, ISetIntervalFilterOutputEmittorModel, ICommonFilterOutputEmittorModel } from '../Models/WidgetRequestModel';
...
export class I2vChartHeaderComponent {
  ...
  @Output() combineFilterOutputEmittor: EventEmitter<ICommonFilterOutputEmittorModel> = new EventEmitter<ICommonFilterOutputEmittorModel>();
  ...
  // DateRange: DateRange;
  private startDate: Date = new Date(new Date($.now()).getFullYear(), new Date($.now()).getMonth(), new Date($.now()).getDate(), 0, 0, 0);
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (!alreadyDone && changes.widgetCustomFiltersValue && changes.widgetCustomFiltersValue.currentValue != changes.widgetCustomFiltersValue.previousValue) {
  ...
  private SetValueAsPerWidgetCustomFiltersValue(initialValue: Boolean = false) {
    if (this.widgetCustomFiltersValue && Object.keys(this.widgetCustomFiltersValue).length > 0 && this.customFilters) {
      ...
      if (this.selectedCustomFilterkey in this.widgetCustomFiltersValue) {
        ...
        CombineFilterOutputEmittorEmitModel['CustomFilterEmitModel'] = { key: this.selectedCustomFilterkey, value: this.selectedCustomFilterValue}
      }
      if ("Time" in this.widgetCustomFiltersValue) {
        ...
        // this.daysFilterOutput.emit(<IDateTimeFilterOutputEmittorModel>{key : this.TimeFilterValue, value : (<ITimeRange>this.widgetCustomFiltersValue["Time"][0].returnValue)});
        CombineFilterOutputEmittorEmitModel['DateFilterEmitModel'] = { key: this.TimeFilterValue, value: (<ITimeRange>this.widgetCustomFiltersValue["Time"][0].returnValue)}
      }
      if ("RefreshInterval" in this.widgetCustomFiltersValue) {
        ...
        // this.refreshIntervalFilterOutput.emit(<ISetIntervalFilterOutputEmittorModel>{key: "RefreshInterval", value : this.refreshIntervalValue});
        CombineFilterOutputEmittorEmitModel['RefreshIntervalEmitModel'] = { key: "RefreshInterval", value: this.refreshIntervalValue}
      }
      this.combineFilterOutputEmittor.next(CombineFilterOutputEmittorEmitModel)
    }
